...
from app.temperature.schema import TemperatureInSchema, TemperatureOutSchema, TemperatureFilterSchema, PeriodType, TemperatureFiteredOutSchema
...
@router.get("", response_model=List[TemperatureFiteredOutSchema], responses={
    200: {"description": "OK", "model": List[TemperatureFiteredOutSchema]},
}, status_code=200)
async def get_temperature(session: Annotated[AsyncSession, Depends(get_db)], filters: TemperatureFilterSchema = Query(...)):
    if filters.type == PeriodType.days:
        ...
        query = (
            select(
                ...
                extract("hour", func.to_timestamp(Temperature.created_at_timestamp)).cast(Integer).label("hour"),
                func.avg(Temperature.temperature).label("temperature")
            )
            .group_by(
                func.to_char(
                ...
                extract("hour", func.to_timestamp(Temperature.created_at_timestamp))
            )
            .order_by(
                func.to_char(
                    func.to_timestamp(Temperature.created_at_timestamp), 
        ...
        query = (
            select(
                ...
                extract("hour", func.to_timestamp(Temperature.created_at_timestamp)).cast(Integer).label("hour"),
                extract("minute", func.to_timestamp(Temperature.created_at_timestamp)).cast(Integer).label("minute"),
                func.avg(Temperature.temperature).label("temperature")
            )
            .group_by(
                func.to_char(
                ...
                extract("hour", func.to_timestamp(Temperature.created_at_timestamp)),
                extract("minute", func.to_timestamp(Temperature.created_at_timestamp))
            )
            .order_by(
                func.to_char(
                    func.to_timestamp(Temperature.created_at_timestamp), 
    ...
    for temperature in temperatures:
        ...
        res_temperature["timestamp"] = datetime(*map(int, temperature["day"].split("-")), hour=temperature.get("hour", 0), minute=temperature.get("minute", 0), tzinfo=UTC).timestamp()
